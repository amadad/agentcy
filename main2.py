...
agency_researcher = AssistantAgent(
    ...
    llm_config={"config_list": config_list},
    system_message=f'''
    You are the Lead Researcher. You MUST rewrite the {user_task} as a query THEN use the research function to answer the questions.
    Delve deep into understanding user pain points, identifying market opportunities, and analyzing prevailing market conditions.
...
user_proxy = UserProxyAgent(
    ...
    system_message="Helpfully guide to successful user outcome. Use save_markdown function to save all conversations # filename: <filename>.",
    function_map={
        "research": research,
        "write_content": write_content,
